...
function geoFail() {
    var input;
	 $("body").prepend('<div class="Search the location"><h1>Rainfall Prediction</h1><span><input type="text" name="input-field" autofocus placeholder="503111 or Kamareddy,"><button type="button" name="search"><i class="fa fa-search" aria-hidden="true"></i> Search</button></span></div>');


    function proccessInput() {
        //The User can easily get any zipcode in the users input and use that first
        input = document.getElementsByName("input-field")[0].value;
        var re = new RegExp(/\d{6}/, "gi") //search the pincode of 6 consecutive numbers
        var matched = input.match(re) //returns an array of each instance of 6 consecutive numbers
        if (matched != null) {
...
function tempToggle(tempInF,app_temp,forecast_arr) {
    ...
    //attaching the click handler to toggle to/from of F/C
    $("#temp").click(function() {
        if (gV.toggled===true) {
...
//working of the temperature
function tempScale(temp) {
    //convert tempaerature bar height
